...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.groupBy((elem: Char) => elem).mapValues(_.size).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match
    ...
    case x::xs => 
      ...
      (for each <- ((for n <- 1 to x._2 yield (x._1, n))) yield combs ++ combs.map(pair => each::pair)).flatten.toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match
    ...
    case yh::ys => 
      val xmap = x.toMap
      if (xmap.contains(yh._1) && ((xmap(yh._1) - yh._2) != 0)) 
      then subtract(xmap.updated(yh._1, (xmap(yh._1) - yh._2)).toList.sorted, ys) 
      else subtract((xmap - yh._1).toList.sorted, ys)  
    ...
    def sentenceAnagramsAux(occurrences: Occurrences): List[Sentence] = 
      if (occurrences.isEmpty) then List(Nil)
        ...
        for 
          ...
          sent <- sentenceAnagramsAux(subtract(occurrences, wordOccurrences(word)))
          if !(combi.isEmpty)
        yield (word::sent)
