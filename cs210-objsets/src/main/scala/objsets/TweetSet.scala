...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = 
    var most: Tweet = elem
    this.foreach(tweet => if(tweet.retweets > most.retweets) then most = tweet)
    most
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = 
    TweetReader.allTweets.filter((tweet: Tweet) => (google.exists((t: String) => tweet.text.contains(t))))
  lazy val appleTweets: TweetSet = 
    TweetReader.allTweets.filter((tweet: Tweet) => (apple.exists((t: String) => tweet.text.contains(t))))
