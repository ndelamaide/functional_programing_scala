...
class RecursiveLanguageSuite extends munit.FunSuite {
  ...
  test("eval") {
    ...
    try {
      ...
      fail("Evaluating a match with an invalid scrutinee did not raise an error.")
    } catch {
  ...
  test("substitution with function capture") {
  ...
  test("substitution with match capture") {
  ...
  override val munitTimeout = scala.concurrent.duration.Duration(200, "s")
}
