...
object RecursiveLanguage {
  ...
  def eval(e: Expr, defs: DefEnv): Expr =
    e match
      ...
      case Match(scrut, empt, h, t, c) => 
        val sc = eval(scrut, defs)
        sc match
          case Empty => eval(empt, defs)
          case Cons(x, xs) => eval(subst(subst(c, t, xs), h, x), defs)
          case _ => error(s"${show(scrut)} is not a list")
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) =>
        if headName == n || tailName == n then
          ...
          if fvs.contains(headName) || fvs.contains(tailName) then
            val head2 = differentName(headName, fvs)
            val caseCons1 = alphaConvert(caseCons, headName, head2)
            val tail2 = differentName(tailName, fvs)
            val caseCons2 = alphaConvert(caseCons1, tailName, tail2)
            Match(scrutinee, caseEmpty, head2, tail2, subst(caseCons2, n, r))
  ...
  def freeVars(e: Expr): Set[String] =
    e match
      ...
      case Match(scrut, empt, h, t, c) => scrut match
        case Empty => freeVars(empt)
        case Cons(x, xs) => freeVars(c) 
        case _ => error(s"${show(scrut)} is not a list")
  ...
  def alphaConvert(e: Expr, n: String, m: String): Expr =
    e match
      ...
      case Match(scrut, empt, h, t, c) => c match
        case Name(s) => if s == h then Match(scrut, empt, h, t, c) 
          else Match(scrut, empt, h, t, alphaConvert(c, n, m))
        case _ => Match(scrut, alphaConvert(empt, n, m), h, t, c)
  ...
  def show(e: Expr): String =
    e match
      ...
      case Cons(x, xs) => s"(${show(x)} :: ${show(xs)})"
      case Empty => "Nil"
      case Match(scrut, empt, h, t, c) => s"""(${show(scrut)} match  
          case Nil => ${show(empt)} 
          case  ${show(Name(h))} :: ${show(Name(t))} => ${show(c)})"""
  ...
  val definitions: DefEnv = Map[String, Expr](
    ...
    "foldLeft" -> Fun("ls", Fun("acc", Fun("f", Match(N("ls"), N("acc"), "x", "xs",
                  Call(Call(Call(N("foldLeft"), N("xs")), Call(Call(N("f"), N("acc")), Name("x"))), N("f")))))),

    // TODO Implement foldRight (analogous to foldLeft, but operate right-to-left)
    "foldRight" -> Fun("ls", Fun("z", Fun("fold", Match(N("ls"), N("z"), "x", "xs",
                        Call(Call(N("fold"), N("x")),
                          Call(Call(Call(N("foldRight"), N("xs")), N("z")), N("fold"))))))),
  )
  ...
  def main(args: Array[String]): Unit =
    ...
    //tracingEval(Call(Call(N("map"),Cons(C(1), Empty)),N("square")), definitions)
}
